...
drawState state
  | screen state == RECORD    = Pictures [drawMaximumScore]
  | screen state == CRIADORES = Pictures [drawCreators]
  | screen state == MENU      = Pictures [drawMenu (menu state)]
  | screen state == GAMEOVER  = Pictures [drawGameOver]
  | screen state == GAMEWIN   = Pictures [drawGameWin]
  | screen state == PAUSED    = Pictures [drawJogo state, drawPause]
  | otherwise                 = Pictures [drawJogo state]

drawJogo :: State -> Picture
drawJogo state 
...
drawPause = pictures [ rect, paused, pressKeySpace, pressKeyBack]
  where
    ...
    pressKeyBack  = color black (translate (-95) (-60) (scale 0.1 0.1 (text "Press Q to go back to Menu")))
